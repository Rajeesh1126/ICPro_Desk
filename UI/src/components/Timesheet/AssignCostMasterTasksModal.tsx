...
const getProjectLabel = (project: SubmissionProject) =>
  [project.code, project.description].filter(Boolean).join(" - ") || `Project ${project.id}`;
...
const AssignCostMasterTasksModal: React.FC<AssignCostMasterTasksModalProps> = ({
    ...
    () =>
      phaseOrder
        .map((phaseName) => ({
          name: phaseName,
          categories: phases
            .filter((phase) => phase.phase === phaseName && phase.cost_category !== null)
            .map((phase) => categoryById.get(phase.cost_category as number))
            .filter((category): category is TimeSheetCostCategory => Boolean(category))
            .filter((category) => (costMastersByCategory.get(category.id)?.length ?? 0) > 0)
  ...
  const allSelectableIds = useMemo(
    () => phasesWithCategories.flatMap((phase) =>
      phase.categories.flatMap((category) =>
        (costMastersByCategory.get(category.id) ?? []).map((costMaster) => costMaster.id),
      ),
    ),
  ...
  const setProjectSelection = (projectId: number, ids: number[], checked: boolean) => {
  ...
  const submitSelection = async () => {
    const payload = Object.entries(selectedByProject).reduce<Record<number, number[]>>(
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="lg">
      ...
      <DialogContent sx={{ height: 560, overflow: "auto", p: 2 }}>
        {loading ? (
          ...
          <Typography color="text.secondary">No assigned projects found for the selected week.</Typography>
        ) : phasesWithCategories.length === 0 ? (
          <Typography color="text.secondary">No mapped cost master items found.</Typography>
        ) : (
          <Stack spacing={1}>
            {projects.map((project) => {
              const projectKey = `project-${project.id}`;
              const projectState = getSelectionState(project.id, allSelectableIds);

              return (
                <Box key={project.id} sx={{ border: "1px solid", borderColor: "divider", borderRadius: 1 }}>
                  <Stack direction="row" alignItems="center" spacing={0.5} sx={{ px: 1, py: 0.75 }}>
                    <IconButton size="small" onClick={() => toggleExpanded(projectKey)}>
                      {expandedKeys.includes(projectKey) ? <ExpandLessIcon /> : <ExpandMoreIcon />}
                    </IconButton>
                    <Checkbox
                      size="small"
                      ...
                      onChange={(event) =>
                        setProjectSelection(project.id, allSelectableIds, event.target.checked)
                      }
                    />
                    <AssignmentOutlinedIcon fontSize="small" color="action" />
                    <Typography fontWeight={600}>{getProjectLabel(project)}</Typography>
                  </Stack>
                  <Collapse in={expandedKeys.includes(projectKey)} timeout="auto" unmountOnExit>
                    <Stack spacing={0.5} sx={{ pb: 1, pl: 4, pr: 1 }}>
                      {phasesWithCategories.map((phase) => {
                        ...
                        const phaseState = getSelectionState(project.id, phaseIds);

                        return (
                          <Box key={phaseKey}>
                            <Stack direction="row" alignItems="center" spacing={0.5}>
                              <IconButton size="small" onClick={() => toggleExpanded(phaseKey)}>
                                {expandedKeys.includes(phaseKey) ? <ExpandLessIcon /> : <ExpandMoreIcon />}
                              </IconButton>
                              <Checkbox
                                size="small"
                                ...
                                onChange={(event) =>
                                  setProjectSelection(project.id, phaseIds, event.target.checked)
                                }
                              />
                              <Typography variant="body2" fontWeight={600}>{phase.name}</Typography>
                            </Stack>
                            <Collapse in={expandedKeys.includes(phaseKey)} timeout="auto" unmountOnExit>
                              <Stack spacing={0.25} sx={{ pl: 4 }}>
                                {phase.categories.map((category) => {
                                  const categoryKey = `${project.id}-${phase.name}-${category.id}`;
                                  const categoryIds = (costMastersByCategory.get(category.id) ?? []).map(
                                    (costMaster) => costMaster.id,
                                  );
                                  const categoryState = getSelectionState(project.id, categoryIds);

                                  return (
                                    <Box key={categoryKey}>
                                      <Stack direction="row" alignItems="center" spacing={0.5}>
                                        <IconButton size="small" onClick={() => toggleExpanded(categoryKey)}>
                                          {expandedKeys.includes(categoryKey) ? <ExpandLessIcon /> : <ExpandMoreIcon />}
                                        </IconButton>
                                        <Checkbox
                                          size="small"
                                          ...
                                          onChange={(event) =>
                                            setProjectSelection(project.id, categoryIds, event.target.checked)
                                          }
                                        />
                                        <Typography variant="body2">{category.name}</Typography>
                                      </Stack>
                                      <Collapse in={expandedKeys.includes(categoryKey)} timeout="auto" unmountOnExit>
                                        <Stack sx={{ pl: 5 }}>
                                          {(costMastersByCategory.get(category.id) ?? []).map((costMaster) => (
                                            <Stack key={costMaster.id} direction="row" alignItems="center" spacing={0.5}>
                                              <Checkbox
                                                size="small"
                                                checked={(selectedByProject[project.id] ?? []).includes(costMaster.id)}
                                                onChange={(event) =>
                                                  setProjectSelection(project.id, [costMaster.id], event.target.checked)
                                                }
                                              />
                                              <Typography variant="body2" color="text.secondary">
                                                {costMaster.name}
      ...
      <DialogActions>
        <Button onClick={onClose} sx={modalActionButtonSx}>
          Cancel
        </Button>
        <Button
          variant="contained"
