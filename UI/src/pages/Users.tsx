...
export default function Users() {
  ...
  const [selectedGroupRow, setSelectedGroupRow] = useState<groupData | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    void api
      ...
      .then((response) => {
        if (!active) return;
        setUsers(
          (Array.isArray(response.data) ? response.data : []) as UsersData[],
        );
      })
      .catch((error) => {
    ...
    async (group: groupData, managerId: number | null) => {
      ...
      try {
        ...
        showNotification({
          type: "success",
          message: managerId
            ? "Manger Mapped successfully."
            : "Managre Updated successfully.",
        });
      } catch (error) {
